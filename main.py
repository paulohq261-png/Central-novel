...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col items-center p-4">
    <div class="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 mt-6">
        <header class="text-center mb-6">
            ...
            <p class="text-slate-400 text-sm mt-1">Powered by Google Gemini</p>
        </header>
        ...
        <div id="loading" class="hidden text-center my-6">
            ...
            <p class="text-slate-400 text-sm mt-2">Dola está buscando...</p>
        </div>
    ...
    <script>
        async function buscarNovel() {
            ...
            if (!query) { alert('Digite o nome da novel!'); return; }
...
def processar():
    ...
    try:
        ...
        prompt = f"Você é a Dola, especialista em novels. O usuário pediu: '{query}'. Faça um resumo e um trecho em português, estilo narrativo."
